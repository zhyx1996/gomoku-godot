...
#define SIZE 15
#define CHARSIZE 3   // UTF-8 下每个字符 3 字节
#define CELLW 2      // 每格显示宽度：2 列让棋盘格接近正方形
...
char aInitDisplayBoardArray[SIZE][SIZE*CELLW*CHARSIZE+1] = 
{
		"┏─┯─┯─┯─┯─┯─┯─┯─┯─┯─┯─┯─┯─┯─┓",
		"┠─┼─┼─┼─┼─┼─┼─┼─┼─┼─┼─┼─┼─┼─┨",
		"┠─┼─┼─┼─┼─┼─┼─┼─┼─┼─┼─┼─┼─┼─┨",
...
//此数组用于显示
char aDisplayBoardArray[SIZE][SIZE*CELLW*CHARSIZE+1];
//此数组用于算法计算使用
int aRecordBoard[SIZE][SIZE];//
char play1Pic[]="⬤";  // 大号实心圆（黑子）
char play2Pic[]="◯";  // 大号空心圆（白子）

//主函数，用于选择模式
void main(){
#ifdef _WIN32
	// 把控制台输入/输出编码切换为 UTF-8，避免棋盘框线乱码
...
void recordtoDisplayArray(void){
//将aRecordBoard中记录的棋子位置，转化到aDisplayBoardArray中
//注意：UTF-8 下棋盘字符与棋子(⬤◯)都是 3 字节；每格占 CELLW=2 列，交叉点位于第 j*CELLW 个字符处
	int i,j;
	for(i=0;i<SIZE;i++)
		strcpy(aDisplayBoardArray[i], aInitDisplayBoardArray[i]);
	for(i=0;i<SIZE;i++)
		for(j=0;j<SIZE;j++){
			if (aRecordBoard[i][j]==1){
				memcpy(&aDisplayBoardArray[i][j*CELLW*CHARSIZE], play1Pic, CHARSIZE);
			}
			if (aRecordBoard[i][j]==2){
				memcpy(&aDisplayBoardArray[i][j*CELLW*CHARSIZE], play2Pic, CHARSIZE);
			}
		}
}
